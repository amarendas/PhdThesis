...
All the papers reviewed use  caster wheel in deriving the dynamic model of a WMR. In this chapter, the modelling is carried out considering the passive wheels in the most general configuration.
 ...
 The concept of the NOC which inherently takes into account the non-holonomic constraint of the wheels has been used.
...
C(\theta,\dot{\theta})&\equiv -T^T(M\dot{T}+WMT)\dot{\theta} \quad :\text{generalized  convective inertia matrix}\\
\tau&\equiv T^Tw^T \quad \quad :\text{ generalized vector of driving forces }
\end{align*}
...
\begin{equation}
\label{velPlat}
 \quad \quad {\dot{o_3}}=[(ar/l)(\dot{\theta_1}-\dot{\theta_2})+(r/2)(\dot{\theta_1}+\dot{\theta_2})]{i}
\end{equation}
...
T_2=\begin{pmatrix}	-i+(r/l)k & -(r/l)k\\  0 &rj \end{pmatrix}\],
\[T_3=\begin{pmatrix}
	\rho\delta & -\rho\delta\\
	r(\lambda i+(1/2))j & r(-\lambda i+(1/2))j
	\end{pmatrix}\]
...
In order to calculate the twists of the caster bracket and the caster wheel, it is necessary to express the  unactuated joint rates, $\dot{\psi_1}$ and $\dot{\phi_1}$, in terms of the actuated joint rate vector $\dot{\theta_a}$. Note here that $\dot{\psi_1}$ denotes the rate of rotation of the bracket body (\#5) about $O_5$ with respect to the platform, and $\dot{\phi_1}$ is the rate of rotation of caster wheel body (\#4) about its axis $e_4$ with respect to the bracket. The velocity of $O_5$ can be expressed in two independent forms, namely, one in terms of the velocity of $O_3$ and the other one in terms of the velocity of $O_5$, i.e.,
\begin{equation}
\dot{o_5}=\dot{o_4}+\omega_5\times(d_1e_4-d2f_4), \quad
\dot{o_5}=\dot{o_4}+\omega_5\times(d_1e_4-d2f_4)
\end{equation}
On equating the above two equations together, and using the rotation matrix between coordinate system $\{i,j,k\}$ and $\{e_4,f_4,k\}$,  the equation in terms $e_4$ and $f_4$, is obtained as
\begin{equation}
...
\subsubsection{Generalized inertia matrix, $I$}
 The equations gives in Section 4.2.1 of the twist of individual body, i.e $t_i=T_i\dot{\theta_a}$ are used to obtain   $t=[t_1^T, t_2^T....t_7^T]^T$ and $t=T\dot\theta$ where  $T=[T_1^T, T_2^T,... T_7]^T$. Since the matrix $M$ is block diagonal, the inertia matrix of the full system  denoted by $I$ is given by
\begin{equation}
...
\subsection{Underactuated case ($d_2=0$)}
...
\[ \lambda= \{-16.7794 + 0.0000i, ~ -1.6103 + 1.8348i, ~ -1..6103 - 1.8348i\}\]. \textbf{\textit{These controller parameters were arrived by trial and error using multiple simulation. The guiding principle behind the selection was to choose one root far from the other two roots so that the system behaves as second order system. The remaining two roots were chosen as complex conjugate with negative real part and near to imaginary axis.  This was to make the steering unit under damped.}}
