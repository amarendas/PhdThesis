...
The dynamic model of the steering system shown in Figure \ref{fig:SteerAsm} is derived  using Lagrangian method. The equations ofmotions are derived under two assumptions. First that the vehicle body \#3 is at rest and second that the mass of links \#8 and \#9 are negligible with respect to other links. The kinematic energy of all the body except \#8 and \#9 are derived next.
...
\end{equation}
 Where, $m_6$ is the mass, $(x^b_6,y^b_6)$ is the coordinate of c.m.  expressed  in the body co-ordinate frame $B:\{i_6,j_6,k_6\}$ as shown in Figure \ref{fig:SteerLink} and $I_{6zz}$ is the moment of inertia in the body coordinate of link \#6.
...
\end{pmatrix}\]
 ...
 \end{equation} where, $l_6$ is the length shown in Figure \ref{fig:SteerLink}  and $\dot\theta_4$ is the spinning rate of the wheel about axis $i_6$.
...
\[K_s=\sum_{i=4..10}K_i\]
using Equations 4.48, 4.49 and 4.50 in the above equation we get
\begin{multline}
...
\subsection{Design of PD controller}
To control the steering of the front wheel a computed torque controller \cite{craig2005introduction} is designed. If $U$ is an auxiliary control input then define
\begin{multline}
\label{eqn:SteerCont}
...
+F_1+F_2
\end{multline}
replacing $F_s$ in Equation \ref{eqn:SteerDyn} with \ref{eqn:SteerCont} and defining $M= m_{10}+\left(m r_o^2+I{zz}+l^2_3m_w \right) $, we get
\begin{equation}
\label{eqn:SteerCont2}
\ddot x =U
\end{equation}
If $x_d$ is the set point and $e(t)=(x(t)-x_d)$ is defined as error in position, we define auxiliary input as  
\begin{equation}
\label{eqn:SteerCont3}
 U=-K_d \dot x -K_p \left(x(t)-x_d \right) -K_i\int e(t)dt \end{equation}
then equation \ref{eqn:SteerCont2} can  be written as 
\[\ddot e(t) +K_d \dot{e}(t) +K_p e(t)+ K_i\int e(t)dt=0 \]
It may be noted that $\dot e(t)=\dot x$ and $\ddot e(t)=\ddot x$, since $x_d=const$. The above equation can be written in state space $\dot{Y}=AY$ form with $ \int e(t)=y_1(t)$ as
\begin{equation}
\begin{pmatrix}
...
\end{pmatrix}
\end{equation}
With $K_d=20,~K_p=60,~K_i=100 $ the system is stable as  the state transition matrix $A$ has negative real part in all eigen values $\lambda$ as given below
\[ \lambda= \{-16.7794 + 0.0000i, ~ -1.6103 + 1.8348i, ~ -1.6103 - 1.8348i\}\].

\subsection{Simulation and results}
The simulation of the  dynamic equation \ref{eqn:SteerDyn} and controller given by equations \ref{eqn:SteerCont} and \ref{eqn:SteerCont3} was carried out with the step change in steering angle of 20mm.  The parameter used for the nominal plant were generated form solid model of the parts and are listed in table \ref{tb:steerPara} The results of the wheel orientations and the actuator effort required are given in Figure \ref{fig:SteerPos} and \ref{fig:SteerEfort} respectively.
...
\begin{figure}
	...
	%\caption{Actuator effort}
\end{figure} 
The simulation results establishes that with $10\%$ error in plant parameters there is practically no deviation in the performance of the controller. It also indicates that the system has a settling delay of $0.8~Sec$. It may also be noted from Equation \ref{eqn:SteerDyn} that the roll velocity of the wheel does not affect the dynamics of the steering system.
\section{Summary}
